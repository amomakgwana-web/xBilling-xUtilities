...
export function UtilitiesDashboard() {
  ...
  const kpis = [
    { label: "Total Meters", value: String(meters.length), sub: "Prepaid · conventional · water", accent: T.brand, icon: IC.meter, nav: "/utilities/meters" },
    { label: "Healthy Meters", value: String(normalCount), sub: `${meters.length - normalCount} need attention`, accent: T.green, icon: IC.shield, nav: "/utilities/meters" },
    { label: "Open Faults", value: String(openFaults.length), sub: `${faults.length} total reported`, accent: T.red, icon: IC.alert, nav: "/utilities/faults" },
  ];
