...
export function Reports() {
  ...
  const load = () => {
    ...
    Promise.all([
      ...
      unwrap<Invoice[]>(supabase.from("invoices").select("*")),
      unwrap<SubsidyApplication[]>(supabase.from("subsidy_applications").select("*")),
      unwrap<Dispute[]>(supabase.from("disputes").select("*")),
    ])
